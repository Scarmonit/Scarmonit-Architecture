...
function summarizeAgent(agent: AgentInfo): string {
  ...
  const take = (startIdx: number) => startIdx === -1 ? '' : lines.slice(startIdx + 1, startIdx + 6).filter(l => l.trim().startsWith('-')).map(l => l.replace(/^[-*]\s*/, '• ')).join('\n');
  const parts = [] as string[];
  if (expertiseIdx !== -1) parts.push('Expertise:\n' + take(expertiseIdx));
  if (alwaysIdx !== -1) parts.push('Always Do:\n' + take(alwaysIdx));
  if (neverIdx !== -1) parts.push('Never Do:\n' + take(neverIdx));
  return `# ${agent.name}\n${parts.join('\n\n')}`.trim();
}
