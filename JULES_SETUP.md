...
While Jules creates Pull Requests, it does not currently support "auto-merge" as a native setting. To achieve automated workflows:

1.  **Jules API Key:** Used to programmatically start Jules sessions or integrate Jules into your tools.
2.  **GitHub Token:** Required if you want to script GitHub actions (like merging) from within a Jules task or a CI pipeline.

**Pro Tip: Auto-Merging**
To automate merging, you can chain commands using the GitHub CLI (`gh`). For example, in a CI environment or a custom script wrapping Jules:

```bash
# 1. Create a PR via Jules (or have Jules do it)
# 2. Enable auto-merge on the resulting PR
gh pr merge <PR-URL> --squash --auto
```

Ensure the environment running these commands has a valid `GITHUB_TOKEN` with `write` permissions for pull requests.
